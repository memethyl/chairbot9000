...
class Perms():
	...
	@perms.command(pass_context=True, description="Set the required role for a certain command.")
	async def set(self, ctx, command: str, role: str):
		...
		if role in [x.name for x in ctx.message.server.roles]:
			config.cfg["main"]["perms"][command] = role
			config.UpdateConfig.save_config(config.cfg)
			content = "Command "+command+" now requires the "+role+" role."
			...
			content = "Could not find the "+role+" role."
			await sendembed(self.bot, channel=ctx.message.channel, color=discord.Colour.dark_red(),
							title="Role Not Found", content=content)
	@perms.command(pass_context=True, description="Remove a specific permission for a command, reverting it back to the default.")
